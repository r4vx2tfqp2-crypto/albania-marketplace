...
export default function SellerOrders() {
  ...
  const generateInvoice = async (order) => {
    ...
    // Logo T in white circle
    doc.setFillColor(...WHITE);
    doc.circle(ml + 6, 9, 6, "F");
    doc.setTextColor(...BLACK);
    doc.setFontSize(9);
    doc.setFont("helvetica", "bold");
    doc.text("T", ml + 6, 11.5, { align: "center" });

    // Brand name
    doc.setTextColor(...WHITE);
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.text("tregu", ml + 16, 11);
    ...
    doc.setFontSize(26);
    doc.setFont("helvetica", "bold");
    doc.text("FATURE", ml, y);

    doc.setFontSize(8);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...GRAY);
    doc.text("Nr:", pageW - mr - 50, 26);
    doc.text("Data:", pageW - mr - 50, 32);
    doc.text("Statusi:", pageW - mr - 50, 38);
    doc.setTextColor(...BLACK);
    doc.setFont("helvetica", "bold");
    doc.text("#" + order.id.slice(0, 8).toUpperCase(), pageW - mr, 26, { align: "right" });
    ...
    doc.setFontSize(7);
    doc.setFont("helvetica", "bold");
    doc.text("SHITESI", ml, y);
    doc.text("BLERESI", ml + cW / 2, y);

    y += 6;
    doc.setTextColor(...BLACK);
    doc.setFontSize(9);
    doc.setFont("helvetica", "bold");
    doc.text(shop?.name || "Dyqani", ml, y);
    doc.text(order.customer_name, ml + cW / 2, y);

    y += 6;
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.setTextColor(...DARK);
    if (shop?.location) { doc.text(shop.location, ml, y); }
    doc.text(order.customer_address + ", " + order.customer_city, ml + cW / 2, y, { maxWidth: cW / 2 });
    y += 6;
    if (shop?.phone) { doc.text(shop.phone, ml, y); }
    doc.text(order.customer_phone, ml + cW / 2, y);
    y += 6;
    doc.setTextColor(...GRAY);
    doc.text("NIPT: _______________", ml, y);
    if (order.customer_email) { doc.text(order.customer_email, ml + cW / 2, y); }
    ...
    doc.setFontSize(7);
    doc.setFont("helvetica", "bold");
    doc.text("NR", ml + 2, y + 6);
    doc.text("PRODUKTI / PERSHKRIMI", ml + 10, y + 6);
    doc.text("SASIA", ml + cW - 54, y + 6);
    doc.text("CMIMI", ml + cW - 36, y + 6);
    doc.text("TOTALI", ml + cW, y + 6, { align: "right" });
    y += 12;
    ...
    // Total row
    doc.setFillColor(...BLACK);
    doc.rect(tX - 4, y - 4, cW - (tX - ml) + 4, 12, "F");
    doc.setTextColor(...WHITE);
    doc.setFontSize(9);
    doc.setFont("helvetica", "bold");
    ...
    // Payment badge
    doc.setFillColor(...VERYLIGHTGRAY);
    doc.roundedRect(ml, y, 60, 10, 2, 2, "F");
    doc.setTextColor(...DARK);
    doc.setFontSize(7.5);
    doc.setFont("helvetica", "bold");
    doc.text("PAGESA ME DOREZIM", ml + 30, y + 6.5, { align: "center" });

    // QR code - right side
    doc.addImage(qrDataUrl, "PNG", ml + cW - 28, y - 2, 28, 28);
    doc.setTextColor(...GRAY);
    doc.setFontSize(6);
    doc.setFont("helvetica", "normal");
    doc.text("Skanoni dyqanin", ml + cW - 14, y + 30, { align: "center" });
    ...
    if (order.notes) {
      ...
      doc.setFontSize(7.5);
      doc.setFont("helvetica", "bold");
      doc.text("Shenime: ", ml, y);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(...DARK);
      doc.text(order.notes, ml + 18, y);
    }

    // ── FOOTER ──
    doc.setDrawColor(...LIGHTGRAY);
    doc.setLineWidth(0.3);
    doc.line(ml, pageH - 25, pageW - mr, pageH - 25);

    // Signature lines
    doc.line(ml, pageH - 12, ml + 45, pageH - 12);
    doc.setTextColor(...GRAY);
    doc.setFontSize(6.5);
    doc.setFont("helvetica", "normal");
    doc.text("Nenshkrimi i Shitesit", ml + 22, pageH - 8, { align: "center" });

    doc.line(pageW - mr - 45, pageH - 12, pageW - mr, pageH - 12);
    doc.text("Nenshkrimi i Bleresit", pageW - mr - 22, pageH - 8, { align: "center" });

    doc.setTextColor(...GRAY);
    doc.setFontSize(7);
    doc.text("Kjo fature eshte gjeneruar nga tregu.store", pageW / 2, pageH - 6, { align: "center" });
