...
export default function SellerDashboard() {
  ...
  const fetchData = async () => {
    ...
    const myOrders = (ordersData || []).filter(order => order.items?.some(item => myProductIds.includes(item.id)));
  ...
  const downloadQR = async (shop) => {
    const url = "https://tregu.store/shop/" + shop.id;
    const canvas = document.createElement("canvas");
    await QRCode.toCanvas(canvas, url, { width: 400, margin: 2, color: { dark: "#1A1916", light: "#FFFFFF" } });
    ...
    ctx.fillText(shop.name, 200, 430);
    ctx.fillStyle = "#1D9E75";
    ctx.font = "14px sans-serif";
    ctx.fillText("tregu.store", 200, 452);
    const link = document.createElement("a");
    link.download = "qr-" + shop.name.toLowerCase().replace(/\s/g, "-") + ".png";
...
>
  🗑️
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
              {shops.map(shop => (
                <div key={shop.id} style={{ display: "flex", alignItems: "center", gap: 12, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: 16 }}>
                  ...
                  <button onClick={() => downloadQR(shop)}
                    style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid var(--border-strong)", fontSize: 12, fontWeight: 500, cursor: "pointer", background: "var(--surface)", fontFamily: "var(--font-body)", color: "var(--text-2)", whiteSpace: "nowrap" }}>
                    QR Code
                  </button>
                </div>
              ))}
      ...
      <div style={{ margin: "20px 0 40px", padding: "20px 24px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        ...
        <a href={"mailto:info@tregu.store?subject=Kerkese%20ndihmes&body=Pershendetje%2C%0A%0AEmail%20im%3A%20" + encodeURIComponent(user?.email || "")}
